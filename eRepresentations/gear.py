...
def Normalize_MinMax(data, global_min, global_max):
    data = np.clip(data, global_min, global_max)
    norm_data = (data - global_min) / (global_max - global_min) * 255.0
    return norm_data.astype(np.float16)
...
def Process_Event_Data(events_file, timestamps_file, output_dir, frame_size, chunk_size):
    ...
    with tables.open_file(events_file, mode='r') as file:
        ...
        event_total = np.full((frame_size[1], frame_size[0]), 0, dtype=np.uint16)
        event_current = np.full((frame_size[1], frame_size[0]), 0, dtype=np.float16)
        event_previous = np.full((frame_size[1], frame_size[0]), 0, dtype=np.float16)
        event_accumulation = np.full((frame_size[1], frame_size[0]), 128, dtype=np.int16)
        ...
        for start in range(0, total_events, chunk_size):
            ...
            for i in range(last, len(timestamps)):
                ...
                for n in range(prev, len(t)):
                    if t[n] <= timestamps[i]:
                        event_accumulation[y[n], x[n]] += ((2 * int(p[n]) - 1) * 2)
                        ...
                        if i != 0 and i != (len(timestamps) - 1):
                            ...
                            event_stack_3d = np.stack((Normalize_MinMax(np.tanh(event_current), -1.0, 1.0), Normalize_MinMax(np.tanh(event_previous), -1.0, 1.0), Normalize_MinMax(np.tanh(event_total), -1.0, 1.0)), axis=-1)
