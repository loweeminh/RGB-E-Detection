...
def Process_Event_Data(events_file, timestamps_file, output_dir, frame_size, chunk_size):
    ...
    with tables.open_file(events_file, mode='r') as file:
        ...
        event_pos = np.full((frame_size[1], frame_size[0]), 0, dtype=np.float32)
        event_neg = np.full((frame_size[1], frame_size[0]), 0, dtype=np.float32)
        ...
        for start in range(0, total_events, chunk_size):
            ...
            for i in range(last, len(timestamps)):
                ...
                for n in range(prev, len(t)):
                    if t[n] <= timestamps[i]:
                        if p[n] == 1:
                            event_pos[y[n], x[n]] = max(event_pos[y[n], x[n]], t[n])
                        else:
                            event_neg[y[n], x[n]] = max(event_neg[y[n], x[n]], t[n])
                    else:
                        if i != 0 and i != (len(timestamps) - 1):
                            min_ts = min(timestamps[i - 1], timestamps[i])
                            max_ts = max(timestamps[i - 1], timestamps[i])
                            nor_pos = (event_pos - min_ts) / (max_ts - min_ts + 1e-6)
                            nor_neg = (event_neg - min_ts) / (max_ts - min_ts + 1e-6)
                            nor_pos = Normalize_MinMax(nor_pos, 1.0)
                            nor_neg = Normalize_MinMax(nor_neg, 1.0)
                            event_stack_3d = np.stack((nor_pos, nor_neg), axis=-1).astype(np.float16)
