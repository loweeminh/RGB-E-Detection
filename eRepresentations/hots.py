...
def Process_Event_Data(events_file, timestamps_file, output_dir, frame_size, chunk_size):
    ...
    with tables.open_file(events_file, mode='r') as file:
        ...
        time_surface_pos = np.full((frame_size[1], frame_size[0]), 0, dtype=np.float16)
        time_surface_neg = np.full((frame_size[1], frame_size[0]), 0, dtype=np.float16)
        ...
        for start in range(0, total_events, chunk_size):
            ...
            for i in range(last, len(timestamps)):
                ...
                for n in range(prev, len(t)):
                    if t[n] <= timestamps[i]:
                        if p[n] == 1:
                            time_surface_pos[y[n], x[n]] = np.exp(-(timestamps[i] - t[n]) / tau)
                        else:
                            time_surface_neg[y[n], x[n]] = np.exp(-(timestamps[i] - t[n]) / tau)
                    else:
                        # Skip the first and last event stacks
                        if i != 0 and i != (len(timestamps) - 1):
                            nor_event_pos = Normalize_MinMax(time_surface_pos, 1.0)
                            nor_event_neg = Normalize_MinMax(time_surface_neg, 1.0)
                            event_stack_3d = np.stack((nor_event_pos, nor_event_neg), axis=-1)
                            Save_Event_Stacks(event_stack_3d, i - 1, output_dir)
                        time_surface_pos.fill(0)
                        time_surface_neg.fill(0)
                        prev = n
                        break_flg = True
                        break
                if not break_flg:
