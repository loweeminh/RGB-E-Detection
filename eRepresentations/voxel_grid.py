...
def Process_Event_Data(events_file, timestamps_file, output_dir, frame_size, chunk_size, bin_num):
    ...
    with tables.open_file(events_file, mode='r') as file:
        ...
        voxel_grid = np.full((frame_size[1], frame_size[0], bin_num), 0, dtype=np.int16)
                ...
                for n in range(prev, len(t)):
                    ...
                    if t[n] <= timestamps[i]:
                        ...
                        if bin_order == bin_num:
                            bin_order = bin_num - 1
                        voxel_grid[y[n], x[n], bin_order] += (2 * int(p[n]) - 1)
                    else:
                        if i != 0 and i != (len(timestamps) - 1):
                            nor_voxel_grid = Normalize_MinMax(voxel_grid, -84, 1660)
                            Save_Event_Stacks(nor_voxel_grid, i - 1, output_dir)
                        voxel_grid.fill(0)
                        t_min = timestamps[i]
                        prev = n
                        break_flg = True
