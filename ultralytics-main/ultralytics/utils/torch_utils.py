...
def get_flops(model, imgsz=640):
    ...
    try:
        ...
        try:
            ...
            im2 = torch.empty((1, p.shape[1], stride, stride), device=p.device)  # input image in BCHW format
            flops = thop.profile(deepcopy(model), inputs=(im, im2), verbose=False)[0] / 1e9 * 2  # stride GFLOPs
