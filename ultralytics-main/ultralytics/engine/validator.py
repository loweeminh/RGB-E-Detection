    ...
    def __call__(self, trainer=None, model=None):
        ...
        if self.training:
            ...
            model.warmup(imgsz=(1 if pt else self.args.batch, self.args.channel_event, imgsz, imgsz), 
                         img2sz=(1 if pt else self.args.batch, self.args.channel_rgb, imgsz, imgsz), 
                         sensor=self.args.sensor)  # warmup
        ...
        for batch_i, batch in enumerate(bar):
            ...
            # Inference
            with dt[1]:
                if self.args.sensor == "event":
                    preds = model(batch["img"], batch["img"], augment=augment)
                elif self.args.sensor == "rgb":
                    preds = model(batch["img2"], batch["img2"], augment=augment)
                else:
