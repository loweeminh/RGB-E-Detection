...
def check_det_dataset(dataset, autodownload=True):
    ...
    # Checks
    for k in "train_event", "val_event", "train_rgb", "val_rgb":
        if k not in data:
            if k != "val_event" or "validation_event" or "val_rgb" or "validation_rgb" not in data:
                raise SyntaxError(
                    emojis(f"{dataset} '{k}:' key missing ❌.\n'train' and 'val' are required in all data YAMLs.")
                )
            LOGGER.info("WARNING ⚠️ renaming data YAML 'validation' key to 'val' to match YOLO format.")
            data["val_event"] = data.pop("validation_event")  # replace 'validation' key with 'val' key
    ...
    for k in "train_event", "val_event", "test_event", "train_rgb", "val_rgb", "test_rgb", "minival":
    ...
    # Parse YAML
    val_event, val_rgb, s = (data.get(x) for x in ("val_event", "val_rgb", "download"))
    if val_event:
        val_event = [Path(x).resolve() for x in (val_event if isinstance(val_event, list) else [val_event])]  # val path
        if not all(x.exists() for x in val_event):
            name = clean_url(dataset)  # dataset name with URL auth stripped
            m = f"\nDataset '{name}' images not found ⚠️, missing path '{[x for x in val_event if not x.exists()][0]}'"
    ...
    if val_rgb:
        ...
        if not all(x.exists() for x in val_rgb):
            name = clean_url(dataset)  # dataset name with URL auth stripped
            m = f"\nDataset '{name}' images not found ⚠️, missing path '{[x for x in val_rgb if not x.exists()][0]}'"
            if s and autodownload:
                LOGGER.warning(m)
            else:
                m += f"\nNote dataset download directory is '{DATASETS_DIR}'. You can update this in '{SETTINGS_FILE}'"
