    ...
    def _mosaic4(self, labels):
        ...
        for i in range(4):
            ...
            if i == 0:  # top left
                ...
                img4_2 = np.full((s * 2, s * 2, img2.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles
            ...
            img4_2[y1a:y2a, x1a:x2a] = img2[y1b:y2b, x1b:x2b]  # img4[ymin:ymax, xmin:xmax]
            padw = x1a - x1b
            padh = y1a - y1b
    ...
    def affine_transform(self, img, img2, border):
        ...
        if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():  # image changed
            if self.perspective:
                ...
                img = cv2.merge([
                    cv2.warpAffine(img[..., i], M[:2], dsize=self.size, borderValue=114)
...
class RandomHSV:
    ...
    def __call__(self, labels):
        ...
        if self.hgain or self.sgain or self.vgain:
            ...
            cv2.cvtColor(im_hsv, cv2.COLOR_HSV2BGR, dst=img2)  # no return needed
        return labels
...
class LetterBox:
    ...
    def __call__(self, labels=None, image=None):
        ...
        img2 = labels.get("img2") if image is None else image
        ...
        padded_channels = [
            cv2.copyMakeBorder(
                img[:, :, i], top, bottom, left, right, cv2.BORDER_CONSTANT, value=114
            )
            for i in range(img.shape[2])
        ]
        ...
        padded_channels2 = [
            cv2.copyMakeBorder(
                img2[:, :, i], top, bottom, left, right, cv2.BORDER_CONSTANT, value=114
            )
            for i in range(img2.shape[2])
        ]
        img2 = np.stack(padded_channels2, axis=-1)
...
class Format:
    ...
    def __call__(self, labels):
        ...
        if self.return_mask:
            ...
            labels["masks"] = masks
        labels["img"] = self._format_img(img, sensor = "event")
        labels["img2"] = self._format_img(img2, sensor = "rgb")
    ...
    # loweeminh
    def _format_img(self, img, sensor="rgb"):
        """
...
def v8_transforms(dataset, imgsz, hyp, stretch=False):
    ...
    return Compose(
        [
            ...
            # MixUp(dataset, pre_transform=None, p=hyp.mixup),
