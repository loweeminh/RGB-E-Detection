...
class BaseDataset(Dataset):
    ...
    def __init__(
        ...
        self.ims, self.ims2, self.im_hw0, self.im_hw = [None] * self.ni, [None] * self.ni, [None] * self.ni, [None] * self.ni
    ...
    def load_image(self, i, rect_mode=True):
        ...
        im, im2, f, f2 = self.ims[i], self.ims2[i], self.im_files[i], self.im_files2[i]
        if im is None:  # not cached in RAM
            im = np.load(f)
            im2 = cv2.imread(f2)
            if im is None or im2 is None:
                raise FileNotFoundError(f"Image Not Found {f} or {f2}")
            ...
            if self.augment:
                self.ims[i], self.ims2[i], self.im_hw0[i], self.im_hw[i] = im, im2, (h0, w0), im.shape[:2]  # im, hw_original, hw_resized
                self.buffer.append(i)
                if 1 < len(self.buffer) >= self.max_buffer_length:  # prevent empty buffer
                    j = self.buffer.pop(0)
                    if self.cache != "ram":
                        self.ims[j], self.ims2[j], self.im_hw0[j], self.im_hw[j] = None, None, None, None

            return im, im2, (h0, w0), im.shape[:2]

        return self.ims[i], self.ims2[i], self.im_hw0[i], self.im_hw[i]
