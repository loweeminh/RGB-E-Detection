...
class LoadImagesAndVideos:
    ...
    def __init__(self, path, path2, sensor, batch=1, vid_stride=1):
        ...
        if self.sensor == "fusion":
            parent2 = None
            if isinstance(path2, str) and Path(path2).suffix == ".txt":  # *.txt file with img/vid/dir on each line
                parent2 = Path(path2).parent2
                path2 = Path(path2).read_text().splitlines()  # list of sources
            files2 = []
            for p in sorted(path2) if isinstance(path2, (list, tuple)) else [path2]:
                a = str(Path(p).absolute())  # do not use .resolve() https://github.com/ultralytics/ultralytics/issues/2912
                if "*" in a:
                    files2.extend(sorted(glob.glob(a, recursive=True)))  # glob
                elif os.path.isdir(a):
                    files2.extend(sorted(glob.glob(os.path.join(a, "*.*"))))  # dir
                elif os.path.isfile(a):
                    files2.append(a)  # files (absolute or relative to CWD)
                elif parent2 and (parent2 / p).is_file():
                    files2.append(str((parent2 / p).absolute()))  # files (relative to *.txt file parent)
                else:
                    raise FileNotFoundError(f"{p} does not exist")

        # Define files as images or videos
        images, images2, videos = [], [], []
        for f in files:
    ...
    def __next__(self):
        ...
        paths, imgs, imgs2, info = [], [], [], []
        while len(imgs) < self.bs:
            if self.count >= self.nf:  # end of file list
                if imgs:
                    return paths, imgs, imgs2, info  # return last partial batch
                else:
                    raise StopIteration
...
class LoadPilAndNumpy:
    ...
    def __next__(self):
        ...
        if self.count == 1:  # loop only once as it's batch inference
            raise StopIteration
        self.count += 1
        return self.paths, self.im0, self.im0_2, [""] * self.bs
