...
class AutoBackend(nn.Module):
    ...
    # loweeminh
    def forward(self, im, im2, augment=False, visualize=False, embed=None):
        """
    ...
    def warmup(self, imgsz=(1, 3, 640, 640), img2sz=(1, 3, 640, 640), sensor = "event"):
        """
        ...
        if any(warmup_types) and (self.device.type != "cpu" or self.triton):
            ...
            # loweeminh
            im2 = torch.empty(*img2sz, dtype=torch.half if self.fp16 else torch.float, device=self.device)  # input
            if sensor == "event":
                for _ in range(2 if self.jit else 1):
                    self.forward(im, im)  # warmup
            elif sensor == "rgb":
                for _ in range(2 if self.jit else 1):
                    self.forward(im2, im2)  # warmup
            else:
                for _ in range(2 if self.jit else 1):
